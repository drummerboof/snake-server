    ...
    _.extend(GameServer.prototype, {
        ...
        createGame: function (options, id) {
            ...
            game.on('gameover', _.bind(function(game) {
                this.io.sockets.of(this.createGameNamespace(id)).emit('game:over', game.serialize());
            }, this));
            this.games[id] = game;
        ...
        _onSocketPlayerJoin: function (socket, data) {
            socket.get('data', _.bind(function (error, socketData) {
                ...
                socket.set('data', socketData, _.bind(function () {
                    try {
                        this.games[socketData.game].addPlayer(new Player(socketData.player));
                        socket.emit('player:join:success', { name: socketData.player });
                    } catch (e) {
