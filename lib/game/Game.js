module.exports = (function Game() {
    ...
    var Game = GameObject.extend({
        ...
        initialize: function (options) {
            this._settings = _.extend({}, this._settings, options || {});
        ...
        removePlayer: function (player) {
            var playerIndex = _.indexOf(this._state.players, player);
            if (playerIndex >= 0) {
        ...
        tick: function () {
            ...
            _.each(this._state.players, function (player) {
                ...
                if (!this._state.matrix.isInBounds(player.getPosition())) {
                    removePlayers[player.getName()] = player;
                    return;
                }
                ...
                _.each(this._state.players, function (otherPlayer, index) {
                    var collision;
                    ...
                    if ((collision = player.collides(otherPlayer)) !== false) {
                        _.each([player, otherPlayer], function (collisionPlayer) {
                            if (collision.equals(collisionPlayer.getPosition())) {
                                removePlayers[collisionPlayer.getName()] = collisionPlayer;
                            }
                        }, this);
                    }
                }, this);
            ...
            if (this.isRunning()) {
                this._timer = setTimeout(this.tick, this.getSpeed());
            }
        },
        ...
        _spawnFood: function () {
            while (this._state.food.length < this._settings.foodMax) {
                ...
                this._state.food.push(new Food(position.getX(), position.getY(), 1));
            }
        },
